...
const errorHandler = (err, req, res, _next) => {
    ...
    if (err.name === 'ValidationError') {
        ...
        errorsTotal.inc({ error_type: 'ValidationError' });
    ...
    if (err.name === 'CastError' && err.kind === 'ObjectId') {
        ...
        errorsTotal.inc({ error_type: 'CastError' });
        return res.status(400).json({
            success: false,
    ...
    if (err.type === 'entity.parse.failed') {
        ...
        errorsTotal.inc({ error_type: 'ParseError' });
        return res.status(400).json({
            success: false,
    ...
    errorsTotal.inc({ error_type: 'ServerError' });
